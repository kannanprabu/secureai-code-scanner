...
class LLMClient:
    ...
    def analyze_per_finding(self, findings: List[Dict], target_path: Path) -> Dict:
        """Analyze ALL findings individually for false positives and exploitability"""
        if not self.enabled:
            logger.warning("LLM not enabled - per-finding analysis skipped")
            return {}
        
        logger.info("Analyzing ALL findings with AI...")
        ...
        # Analyze ALL findings (CRITICAL, HIGH, MEDIUM, LOW)
        findings_to_analyze = (
            critical_findings +           # ALL CRITICAL
            high_findings +               # ALL HIGH
            medium_findings +             # ALL MEDIUM
            low_findings                  # ALL LOW
        )
        
        total_to_analyze = len(findings_to_analyze)
        logger.info(f"Analyzing {total_to_analyze} findings: "
                   f"{len(critical_findings)} CRITICAL, "
        ...
        logger.info(f"Sending ALL {len(findings_data)} findings to LLM for analysis")
        
        # Split into batches (LLM context limits)
...
CRITICAL: Use the exact "key" values from the findings above. Respond ONLY with the JSON object. No backticks, no markdown."""
        ...
        logger.info(f"✓ Coverage: 100% - All findings analyzed!")
...
class EnhancedCodeReviewAgent:
    ...
    def run_semgrep_scan(self, target_path: Path) -> Tuple[bool, List[Dict]]:
        """Run Semgrep scan with comprehensive rules - MERGES all configs for maximum findings"""
        try:
            logger.info(f"Running comprehensive Semgrep scan on {target_path}...")
            logger.info("Strategy: Try multiple configs and MERGE results for maximum coverage")
            ...
            # Dictionary to store unique findings (deduplicated by file:line:rule)
            all_findings = {}
                ...
                try:
                    ...
                    if result.stdout and result.stdout.strip():
                        try:
                            ...
                            if results:
                                # Add findings to merged set (deduplicate by file:line:rule)
                                ...
                                for finding in results:
                                    ...
                                    key = f"{file_path}:{line_num}:{rule}"
                                    
                                    # Only add if not already present (first occurrence wins)
                                    if key not in all_findings:
                                        all_findings[key] = finding
                                
                                new_findings = len(all_findings) - before_count
                                logger.info(f"✓ {config['name']}: Added {new_findings} new unique findings "
                                          f"({len(results)} total from this config, {len(all_findings)} unique overall)")
                                successful_configs += 1
                            else:
                                logger.info(f"  {config['name']}: No findings")
            ...
            # Convert merged findings back to list
            merged_results = list(all_findings.values())
            
            # Check if we got any results
            if not merged_results:
                logger.error("All Semgrep configs failed or found nothing.")
            ...
            logger.info(f"✓ MERGED RESULTS: {len(merged_results)} unique findings from {successful_configs}/{len(configs)} configs")
    ...
    def _write_text_report(self, f, analysis: Dict, llm_analysis: Dict, target_name: str):
        ...
        f.write("\nNOTE: ALL findings receive analysis (LLM or intelligent fallback).\n")
        f.write("Every CRITICAL, HIGH, MEDIUM, and LOW issue is reviewed.\n")
        f.write("\n")
        ...
        # Medium Priority Issues with FULL ANALYSIS
        if analysis['medium_priority']:
            f.write("=" * 100 + "\n")
            f.write("MEDIUM PRIORITY ISSUES (NEXT RELEASE)\n")
            f.write("=" * 100 + "\n\n")
            
            f.write(f"Total MEDIUM issues: {len(analysis['medium_priority'])}\n")
            f.write("All MEDIUM issues analyzed with AI:\n\n")
            
            for idx, issue in enumerate(analysis['medium_priority'], 1):
                f.write(f"Finding {idx}\n")
                f.write(f"Severity: {issue['severity']}\n")
                f.write(f"Location: {issue['file']}:{issue['line']}\n")
                ...
                analysis_text = issue.get('llm_analysis', '')
                if analysis_text and analysis_text not in ['AI analysis pending', 'AI analysis not available']:
                    f.write(f"{analysis_text}\n")
                else:
                    f.write(f"Review this vulnerability carefully. {issue.get('message', '')}\n")
        ...
        # Low Priority Issues with FULL ANALYSIS
        if analysis['low_priority']:
            f.write("=" * 100 + "\n")
            f.write("LOW PRIORITY ISSUES (CODE QUALITY)\n")
            f.write("=" * 100 + "\n\n")
            
            f.write(f"Total LOW issues: {len(analysis['low_priority'])}\n")
            f.write("All LOW issues analyzed with AI:\n\n")
            
            for idx, issue in enumerate(analysis['low_priority'], 1):
                f.write(f"Finding {idx}\n")
                f.write(f"Severity: {issue['severity']}\n")
                f.write(f"Location: {issue['file']}:{issue['line']}\n")
                f.write(f"Rule: {issue['rule']}\n")
                ...
                if analysis_text and analysis_text not in ['AI analysis pending', 'AI analysis not available']:
                    ...
                    f.write(f"Code quality issue. {issue.get('message', '')}\n")
...
def main():
    parser = argparse.ArgumentParser(
        description="SecureCodeAI - LLM-Enhanced Security Code Review"
    )
    
    group = parser.add_mutually_exclusive_group(required=True)
    group.add_argument("--local", type=str, help="Path to local directory to scan")
    group.add_argument("--github", type=str, help="GitHub repository URL (e.g., https://github.com/user/repo)")
    group.add_argument("--repo", type=str, help="Any Git repository URL (Azure DevOps, GitLab, Bitbucket, etc.)")
    
    parser.add_argument("--output-dir", type=str, default="scan_results", help="Output directory for reports")
    ...
    print("\n" + "=" * 100)
    print("SecureCodeAI - LLM-Enhanced Security Code Review")
    print("=" * 100)
    print("\nFeatures:")
    ...
    try:
        if args.local:
            ...
            result = agent.scan_github_repo(args.repo)  # Same function works for any git repo
